...
#include <gtk/gtk.h>
#include "gtkembedfilesel.h"

#define MIXED_LIST_WIDTH   180
#define MIXED_LIST_HEIGHT  180
...
static void gtk_embed_file_selection_class_init    (GtkEmbedFileSelectionClass *klass);
static void gtk_embed_file_selection_init          (GtkEmbedFileSelection      *filesel);
static void gtk_embed_file_selection_destroy       (GtkObject             *object);
static gint gtk_embed_file_selection_key_press     (GtkWidget             *widget,
					      GdkEventKey           *event,
					      gpointer               user_data);
static void
gtk_embed_file_selection_abort (GtkEmbedFileSelection *fs);

static void gtk_embed_file_selection_mixed_button (GtkWidget *widget,
					   gint row, 
					   gint column, 
					   GdkEventButton *bevent,
					   gpointer data);

static void gtk_embed_file_selection_populate      (GtkEmbedFileSelection      *fs,
					      gchar                 *rel_path,
					      gint                   try_complete);

static void gtk_embed_file_selection_update_history_menu (GtkEmbedFileSelection       *fs,
						    gchar                  *current_dir);

static void gtk_embed_file_selection_create_dir (GtkWidget *widget, gpointer data);
static void gtk_embed_file_selection_delete_file (GtkWidget *widget, gpointer data);
static void gtk_embed_file_selection_rename_file (GtkWidget *widget, gpointer data);
...
GtkType
gtk_embed_file_selection_get_type (void)
{
  static GtkType embed_file_selection_type = 0;

  if (!embed_file_selection_type)
    {
      static const GtkTypeInfo filesel_info =
      {
	"GtkEmbedFileSelection",
	sizeof (GtkEmbedFileSelection),
	sizeof (GtkEmbedFileSelectionClass),
	(GtkClassInitFunc) gtk_embed_file_selection_class_init,
	(GtkObjectInitFunc) gtk_embed_file_selection_init,
	/* reserved_1 */ NULL,
	/* reserved_2 */ NULL,
        (GtkClassInitFunc) NULL,
      };

      embed_file_selection_type = gtk_type_unique (GTK_TYPE_VBOX, &filesel_info);
    }

  return embed_file_selection_type;
}

static void
gtk_embed_file_selection_class_init (GtkEmbedFileSelectionClass *class)
{
  ...
  object_class->destroy = gtk_embed_file_selection_destroy;
}

static void
gtk_embed_file_selection_init (GtkEmbedFileSelection *filesel)
{
  ...
  gtk_box_pack_start (GTK_BOX (filesel), filesel->button_area, 
		      FALSE, FALSE, 0);
  gtk_widget_show (filesel->button_area);
  
  gtk_embed_file_selection_show_fileop_buttons(filesel);
  ...
  mixed_title[0] = N_("Files");
  mixed_title[1] = NULL;
  filesel->mixed_list = gtk_clist_new_with_titles (1, (gchar**) mixed_title);
  gtk_widget_set_usize (filesel->mixed_list, MIXED_LIST_WIDTH, MIXED_LIST_HEIGHT);
  gtk_signal_connect (GTK_OBJECT (filesel->mixed_list), "select_row",
		      (GtkSignalFunc) gtk_embed_file_selection_mixed_button, 
		      (gpointer) filesel);
  gtk_clist_column_titles_passive (GTK_CLIST (filesel->mixed_list));

  scrolled_win = gtk_scrolled_window_new (NULL, NULL);
  gtk_container_add (GTK_CONTAINER (scrolled_win), filesel->mixed_list);
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_win),
				  GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
  gtk_container_set_border_width (GTK_CONTAINER (scrolled_win), 5);
  gtk_box_pack_start (GTK_BOX (filesel), scrolled_win, TRUE, TRUE, 0);
  gtk_widget_show (filesel->mixed_list);
  gtk_widget_show (scrolled_win);

  /*  The selection entry widget  */
  entry_vbox = gtk_vbox_new (FALSE, 2);
  gtk_box_pack_end (GTK_BOX (filesel), entry_vbox, FALSE, FALSE, 0);
  ...
  gtk_signal_connect (GTK_OBJECT (filesel->selection_entry), "key_press_event",
		      (GtkSignalFunc) gtk_embed_file_selection_key_press, filesel);
  ...
  if (!cmpl_state_okay (filesel->cmpl_state))
    {
      ...
      sprintf (err_buf, N_("Directory unreadable: %s"), cmpl_strerror (cmpl_errno));

      gtk_label_set_text (GTK_LABEL (filesel->selection_text), err_buf);
    }
  else
    {
      gtk_embed_file_selection_populate (filesel, "", FALSE);
    }
...
GtkWidget*
gtk_embed_file_selection_new ()
{
  GtkEmbedFileSelection *filesel;

  filesel = gtk_type_new (GTK_TYPE_EMBED_FILE_SELECTION);
  return GTK_WIDGET (filesel);
}

void
gtk_embed_file_selection_show_fileop_buttons (GtkEmbedFileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_EMBED_FILE_SELECTION (filesel));
    
  /* delete, create directory, and rename */
  if (!filesel->fileop_c_dir) 
    {
      filesel->fileop_c_dir = gtk_button_new_with_label (N_("Create Dir"));
      gtk_signal_connect (GTK_OBJECT (filesel->fileop_c_dir), "clicked",
			  (GtkSignalFunc) gtk_embed_file_selection_create_dir, 
			  (gpointer) filesel);
  ...
  if (!filesel->fileop_del_file) 
    {
      filesel->fileop_del_file = gtk_button_new_with_label (N_("Delete File"));
      gtk_signal_connect (GTK_OBJECT (filesel->fileop_del_file), "clicked",
			  (GtkSignalFunc) gtk_embed_file_selection_delete_file, 
			  (gpointer) filesel);
      gtk_box_pack_start (GTK_BOX (filesel->button_area), 
  ...
  if (!filesel->fileop_ren_file)
    {
      filesel->fileop_ren_file = gtk_button_new_with_label (N_("Rename File"));
      gtk_signal_connect (GTK_OBJECT (filesel->fileop_ren_file), "clicked",
			  (GtkSignalFunc) gtk_embed_file_selection_rename_file, 
			  (gpointer) filesel);
      gtk_box_pack_start (GTK_BOX (filesel->button_area), 
...
void       
gtk_embed_file_selection_hide_fileop_buttons (GtkEmbedFileSelection *filesel)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_EMBED_FILE_SELECTION (filesel));
...
void
gtk_embed_file_selection_set_filename (GtkEmbedFileSelection *filesel,
				 const gchar      *filename)
{
  ...
  gtk_embed_file_selection_populate (filesel, buf, FALSE);
...
gchar*
gtk_embed_file_selection_get_filename (GtkEmbedFileSelection *filesel)
{
  ...
  g_return_val_if_fail (GTK_IS_EMBED_FILE_SELECTION (filesel), nothing);
...
void
gtk_embed_file_selection_complete (GtkEmbedFileSelection *filesel,
			     const gchar      *pattern)
{
  g_return_if_fail (filesel != NULL);
  g_return_if_fail (GTK_IS_EMBED_FILE_SELECTION (filesel));
  ...
  gtk_embed_file_selection_populate (filesel, (gchar*) pattern, TRUE);
}

static void
gtk_embed_file_selection_destroy (GtkObject *object)
{
  GtkEmbedFileSelection *filesel;
  GList *list;
  ...
  g_return_if_fail (GTK_IS_EMBED_FILE_SELECTION (object));

  filesel = GTK_EMBED_FILE_SELECTION (object);
...
static void
gtk_embed_file_selection_fileop_error (gchar *error_message)
{
  GtkWidget *label;
  GtkWidget *vbox;
  GtkWidget *button;
  ...
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_embed_file_selection_fileop_destroy, 
		      (gpointer) fs);
  */
  gtk_window_set_title (GTK_WINDOW (dialog), N_("Error"));
  ...
  /* close button */
  button = gtk_button_new_with_label (N_("Close"));
...
static void
gtk_embed_file_selection_fileop_destroy (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;

  g_return_if_fail (fs != NULL);
  g_return_if_fail (GTK_IS_EMBED_FILE_SELECTION (fs));
...
static void
gtk_embed_file_selection_create_dir_confirmed (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  gchar *dirname;
  gchar *path;
  ...
  if ( (mkdir (full_path, 0755) < 0) ) 
    {
      ...
      gtk_embed_file_selection_fileop_error (buf);
    }
  g_free (full_path);
  
  gtk_widget_destroy (fs->fileop_dialog);
  gtk_embed_file_selection_populate (fs, "", FALSE);
}
  
static void
gtk_embed_file_selection_create_dir (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  GtkWidget *label;
  GtkWidget *dialog;
  ...
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_embed_file_selection_fileop_destroy, 
		      (gpointer) fs);
  gtk_window_set_title (GTK_WINDOW (dialog), N_("Create Directory"));
  gtk_window_set_position (GTK_WINDOW (dialog), GTK_WIN_POS_MOUSE);
...
/*  if (GTK_WINDOW(fs)->modal) */
  ...
  label = gtk_label_new(N_("Directory name:"));
  gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.0);
  gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 5);
  gtk_widget_show(label);
  ...
  /* buttons */
  button = gtk_button_new_with_label (N_("Create"));
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_embed_file_selection_create_dir_confirmed, 
		      (gpointer) fs);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static void
gtk_embed_file_selection_delete_file_confirmed (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  CompletionState *cmpl_state;
  gchar *path;
...
static void
gtk_embed_file_selection_delete_file (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  GtkWidget *label;
  GtkWidget *vbox;
  ...
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_embed_file_selection_fileop_destroy, 
		      (gpointer) fs);
  gtk_window_set_title (GTK_WINDOW (dialog), N_("Delete File"));
  gtk_window_set_position (GTK_WINDOW (dialog), GTK_WIN_POS_MOUSE);
...
/*  if (GTK_WINDOW(fs)->modal) */
  ...
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_embed_file_selection_delete_file_confirmed, 
		      (gpointer) fs);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static void
gtk_embed_file_selection_rename_file_confirmed (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  gchar *buf;
  gchar *file;
...
static void
gtk_embed_file_selection_rename_file (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  GtkWidget *label;
  GtkWidget *dialog;
  ...
  gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
		      (GtkSignalFunc) gtk_embed_file_selection_fileop_destroy, 
		      (gpointer) fs);
  gtk_window_set_title (GTK_WINDOW (dialog), N_("Rename File"));
  gtk_window_set_position (GTK_WINDOW (dialog), GTK_WIN_POS_MOUSE);
...
/*  if (GTK_WINDOW(fs)->modal) */
  ...
  /* buttons */
  button = gtk_button_new_with_label (N_("Rename"));
  gtk_signal_connect (GTK_OBJECT (button), "clicked",
		      (GtkSignalFunc) gtk_embed_file_selection_rename_file_confirmed, 
		      (gpointer) fs);
  gtk_box_pack_start(GTK_BOX(GTK_DIALOG(dialog)->action_area),
		     button, TRUE, TRUE, 0);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
...
static gint
gtk_embed_file_selection_key_press (GtkWidget   *widget,
			      GdkEventKey *event,
			      gpointer     user_data)
{
  GtkEmbedFileSelection *fs;
  char *text;
  ...
  if (event->keyval == GDK_Tab)
    {
      fs = GTK_EMBED_FILE_SELECTION (user_data);
      text = gtk_entry_get_text (GTK_ENTRY (fs->selection_entry));

      text = g_strdup (text);

      gtk_embed_file_selection_populate (fs, text, TRUE);
...
static void
gtk_embed_file_selection_history_callback (GtkWidget *widget, gpointer data)
{
  GtkEmbedFileSelection *fs = data;
  HistoryCallbackArg *callback_arg;
  GList *list;
  ...
  while (list) {
    ...
    if (callback_arg->menu_item == widget)
      {
	gtk_embed_file_selection_populate (fs, callback_arg->directory, FALSE);
	break;
      }
...
static void 
gtk_embed_file_selection_update_history_menu (GtkEmbedFileSelection *fs,
  ...
  for (i = dir_len; i >= 0; i--)
      ...
      if ( (current_dir[i] == '/') || (i == dir_len))
	{
	  ...
	  gtk_signal_connect (GTK_OBJECT (menu_item), "activate",
			      (GtkSignalFunc) gtk_embed_file_selection_history_callback,
			      (gpointer) fs);
...
static void
gtk_embed_file_selection_mixed_button (GtkWidget *widget,
			       gint row, 
			       gint column, 
			       GdkEventButton *bevent,
			       gpointer user_data)
{
  GtkEmbedFileSelection *fs = NULL;
  ...
  gtk_clist_get_text (GTK_CLIST (fs->mixed_list), row, 0, &temp);
  ...
  if (filename)
	...
	switch (bevent->type)
	  ...
	  case GDK_2BUTTON_PRESS:
  		if ((gint)gtk_clist_get_row_data(GTK_CLIST(fs->mixed_list),row)==0) gtk_embed_file_selection_populate (fs, filename, FALSE);
	    break;
...
static void
gtk_embed_file_selection_populate (GtkEmbedFileSelection *fs,
			     gchar            *rel_path,
			     gint              try_complete)
{
  CompletionState *cmpl_state;
  ...
  if (!cmpl_state_okay (cmpl_state))
    {
      ...
      gtk_embed_file_selection_abort (fs);
      return;
    }
  g_assert (cmpl_state->reference_dir);

  gtk_clist_freeze (GTK_CLIST (fs->mixed_list));
  gtk_clist_clear (GTK_CLIST (fs->mixed_list));

  /* Set the mixed_list to include ./ and ../ */
  text[1] = NULL;
  text[0] = "./";
  row=gtk_clist_append (GTK_CLIST (fs->mixed_list), text);
  gtk_clist_set_row_data (GTK_CLIST (fs->mixed_list), row, (gpointer)0);
  ...
  mixed_list_width = gdk_string_width(fs->mixed_list->style->font,"../");
  gtk_clist_set_column_width(GTK_CLIST(fs->mixed_list),0,mixed_list_width);
	  ...
	  text[0] = filename;
		...
		/* directory */
              if (strcmp (filename, "./") != 0 &&
                  strcmp (filename, "../") != 0)
		{
		dirs=g_list_append(dirs,(gpointer)g_strdup(filename));
 		}
	    }
          else
	    {
		/* file */
		files=g_list_append(files,(gpointer)g_strdup(filename));
        }
	}
  ...
  while(l)
	{
    text[0] = (gchar *)l->data;
	width = gdk_string_width(fs->mixed_list->style->font,text[0]);
    if(width > mixed_list_width) mixed_list_width = width;
	row=gtk_clist_append(GTK_CLIST (fs->mixed_list), text);
	gtk_clist_set_row_data (GTK_CLIST (fs->mixed_list), row, (gpointer)0);
    g_free((gchar *)l->data);
	l=l->next;
  ...
  while(l)
    ...
    if(width > mixed_list_width) mixed_list_width = width;
    row=gtk_clist_append(GTK_CLIST (fs->mixed_list), text);
    gtk_clist_set_row_data (GTK_CLIST (fs->mixed_list), row, (gpointer)1);
    g_free((gchar *)l->data);
	l=l->next;
	}
  g_list_free(files);
  ...
  gtk_clist_set_column_width(GTK_CLIST(fs->mixed_list),0, mixed_list_width);
  gtk_clist_thaw (GTK_CLIST (fs->mixed_list));
      ...
      if (cmpl_updated_text (cmpl_state)[0])
        {
	      ...
	      gchar* dir_name = g_strdup (cmpl_updated_text (cmpl_state));
              ...
              gtk_embed_file_selection_populate (fs, dir_name, TRUE);
  ...
  if (!did_recurse)
	...
	gtk_entry_set_position (GTK_ENTRY (fs->selection_entry), selection_index);
      ...
      if (fs->selection_entry)
	{
	  sel_text = g_strconcat (N_("Selection: "),
	...
	{
	  gtk_embed_file_selection_update_history_menu (fs, cmpl_reference_position (cmpl_state));
	}

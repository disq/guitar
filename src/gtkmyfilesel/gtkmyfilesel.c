...
#define SELECTION_LIST_WIDTH  180
#define SELECTION_LIST_HEIGHT 180
...
static void       gtk_my_file_selection_add_item (GtkMyFileSelection *filesel);
static void       gtk_my_file_selection_remove_item (GtkMyFileSelection *filesel);

static void
gtk_my_file_selection_selected_list_button (GtkWidget *widget,
			       gint row, 
			       gint column, 
			       GdkEventButton *bevent,
			       gpointer user_data);
...
{
  static GtkType my_file_selection_type = 0;

  if (!my_file_selection_type)
    {
	...
	/* reserved_2 */ NULL,
      ...
      my_file_selection_type = gtk_type_unique (GTK_TYPE_WINDOW, &filesel_info);
    }

  return my_file_selection_type;
}
...
/* FIXME */
  gtk_clist_set_selection_mode (GTK_CLIST (filesel->file_list), GTK_SELECTION_EXTENDED);
  ...
  selbut_vbox = gtk_vbox_new (FALSE, 10);
  gtk_box_pack_start (GTK_BOX(list_hbox), selbut_vbox, FALSE, TRUE, 0);
  gtk_widget_show (selbut_vbox);

  filesel->add_button = gtk_button_new_with_label ("->");
  gtk_box_pack_start(GTK_BOX (selbut_vbox), filesel->add_button, FALSE, FALSE, 0);
  gtk_signal_connect_object (GTK_OBJECT (filesel->add_button), "clicked",
		      (GtkSignalFunc) gtk_my_file_selection_add_item, 
		      (gpointer) filesel);
  gtk_widget_show (filesel->add_button);

  filesel->remove_button = gtk_button_new_with_label ("<-");
  gtk_box_pack_start(GTK_BOX (selbut_vbox), filesel->remove_button, FALSE, FALSE, 0);
  gtk_signal_connect_object (GTK_OBJECT (filesel->remove_button), "clicked",
		      (GtkSignalFunc) gtk_my_file_selection_remove_item,
			  (gpointer) filesel);
  gtk_widget_show (filesel->remove_button);

  /* The selections clist */
  selection_title[0] = N_("Selected files");
  selection_title[1] = NULL;
  filesel->selected_list = gtk_clist_new_with_titles (1, (gchar**) selection_title);
  gtk_widget_set_usize (filesel->selected_list, SELECTION_LIST_WIDTH, SELECTION_LIST_HEIGHT);
  gtk_clist_column_titles_passive (GTK_CLIST (filesel->selected_list));
  gtk_clist_set_selection_mode (GTK_CLIST (filesel->selected_list), GTK_SELECTION_EXTENDED);
  gtk_signal_connect (GTK_OBJECT (filesel->selected_list), "select_row",
		      (GtkSignalFunc) gtk_my_file_selection_selected_list_button, 
		      (gpointer) filesel);

  scrolled_win = gtk_scrolled_window_new (NULL, NULL);
  gtk_container_add (GTK_CONTAINER (scrolled_win), filesel->selected_list);
  gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (scrolled_win),
				  GTK_POLICY_AUTOMATIC, GTK_POLICY_ALWAYS);
  gtk_container_set_border_width (GTK_CONTAINER (scrolled_win), 5);
  gtk_box_pack_start (GTK_BOX (list_hbox), scrolled_win, TRUE, TRUE, 0);
  gtk_widget_show (filesel->selected_list);
  gtk_widget_show (scrolled_win);
  ...
  /*  The OK button  */
  filesel->ok_button = gtk_button_new_with_label (N_("OK"));
  ...
  /*  The Cancel button  */
  filesel->cancel_button = gtk_button_new_with_label (N_("Cancel"));
...
/*	    gtk_button_clicked (GTK_BUTTON (fs->ok_button)); */
	    gtk_button_clicked (GTK_BUTTON (fs->add_button));
	    break;
			       ...
			       gpointer user_data)
{
  ...
  if (bevent)
	if (bevent->type==GDK_2BUTTON_PRESS)
	  gtk_clist_remove(GTK_CLIST(fs->selected_list),row);
...
static void
gtk_my_file_selection_add_item (GtkMyFileSelection *filesel)
{
gchar *fn;
gint rows;
gint i;
...
gboolean x;
  ...
  g=GTK_CLIST(filesel->file_list)->selection;
  u=gtk_my_file_selection_get_filename(filesel);
  if (g!=NULL) t=NULL;
  ...
  if (g!=NULL)
	{
	  gtk_clist_get_text (GTK_CLIST(filesel->file_list), (gint)g->data, 0, &t);
	  fn = cmpl_completion_fullname (t, filesel->cmpl_state);
	}
	else
	{
	  fn=u;
	...
	if (access(fn,F_OK)!=0) return;

	rows=GTK_CLIST(filesel->selected_list)->rows;

	x=TRUE;
	for(i=0;i<rows;i++)
	  {
	  gtk_clist_get_text(GTK_CLIST(filesel->selected_list),i,0,&t);
	  if (strcmp(fn,t)==0) { x=FALSE; break; }
	  }
	if (x)
	  {
	  txt[0] = fn;
	  txt[1] = NULL;
      gtk_clist_append(GTK_CLIST(filesel->selected_list), txt);
	  }
	if (g!=NULL) g=g->next;
			else u=NULL;
  }
  gtk_my_file_selection_set_filename(filesel,"");
}


static void
gtk_my_file_selection_remove_item (GtkMyFileSelection *filesel)
{
GList *list;

  list=GTK_CLIST(filesel->selected_list)->selection;
  while(list)
	{
	gtk_clist_remove(GTK_CLIST(filesel->selected_list),(gint)list->data);
	list=GTK_CLIST(filesel->selected_list)->selection;
	}
